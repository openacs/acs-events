...
</ul><h3>II. Introduction</h3><p>The ACS events service is primarily intended for use by writers
...
<i>objects</i>.</p><p>The package doesn't provide for any interpretation of events,
...
provides no UI support.</p><p>Possible application domains include include calendaring, room
reservation, scheduling, project management, and event
registration.</p><p>The requirements break the functionality into four main areas:
...
activities, their properties, and their associated objects.</p><p>
...
that need recurrences added to the DB.</p><h3>III. Historical Considerations</h3><p>There are number of historical considerations surrounding the
...
expanded.</p><p>One key consideration is exactly how recurring events are
...
recurrence.</p><p>I favored the second approach for the following reasons. First,
...
recurrence.</p><p>I think it may be faster in Oracle even with a stored proc, at
...
displayed.</p><p>Another reason is that the first approach, to insert only a
...
proc.</p><p>This is not an unreasonable possibility and is in fact how Sloan
...
attached to it.</p><p>However, there are drawbacks to this approach. First, it will be
...
swamping the database.</p><p>In the ACS 3.4 Calendar Package, this was partially resolved in
...
seems to have no lower limit, but an upper limit of 2031.</p><p>The 4.0 ACS Events service doesn't enforce a particular policy
...
<b>partially_populated_events</b> view.</p><p>
...
limit.</p><p>It is the application's responsibility to determine if
...
instances.</p><p>Another historical consideration is the choice of values for
...
intervals is guaranteed to be a positive integer.</p><p>For the days_of_week column, the representation chosen, a
...
can be stored directly into the DB.</p><h3>IV. Competitive Analysis</h3><p>Since this is a low level service package, there is no direct
competition.</p><h3>V. Design Tradeoffs</h3><p>Because this is a service package, tradeoffs were made only in
...
following areas:</p><h4>Maintainability</h4><p>To simplify the package as much as possible, a number of
...
package</p><p>There was one significant, fairly complex feature that was
...
model across the toolkit.</p><h4>Reusability</h4><p>Much thought was given to the needs of applications most likely
to use this service, such as calendar, events, and room
reservations. This has led to a well defined API which should be
reusable by most applications that are concerned by events.</p><h4>Testability</h4><p>Because the API consists of well defined PL/SQL functions, it
should be fairly easy to build a test suite using the PL/SQL
testing tools.</p><h3>VI. Data Model and API Discussion</h3><p>The data model and PL/SQL API encapsulate the four main
...
wrappers around PL/SQL functions and procedures.</p><h4>Events</h4><p>This is the main abstraction in the package. <tt>acs_event</tt>
...
particular event recurs.</p><h4>Time Interval Sets</h4><p>Because time interval sets are so simple, there is no need to
...
additional manipulation functions.</p><h4>Activities</h4><p>This is the secondary abstraction in the package.
...
attributes and mappings.</p><h4>Recurrences</h4><p>Since recurrences are always associated with events, there
seemed to be no need to make them objects. The information that
determines how an event recurs is stored in the
<tt>event_recurrences</tt> table.</p><p>The <tt>event_recurrence</tt> package defines <tt>new</tt>,
<tt>delete</tt>, and other procedures related to recurrences. The
key procedure is <tt>insert_events</tt>.</p><p>A view, <tt>partially_populated_events</tt>, is created which
...
--><h3>VIII. User Interface</h3><p>This package does not provide a UI.</p><h3>IX. Configuration/Parameters</h3><p>There are no parameters for this package.</p><h3>X. Future Improvements/Areas of Likely Change</h3><p>If the system presently lacks useful/desirable features, note
